...
public class Sample {
    ...
    private double[] decodeValues = null;
    ...
    /*
     * Try to set the defaultAudioFileReaderClass to JavaSoundAudioFile if
     * available, and if not, use WavFileReaderWriter which should always be
     * available in beads_main.
     */
    static {
        try {
            defaultAudioFileReaderClass = (Class<? extends AudioFileReader>) Class
                    .forName("net.beadsproject.beads.data.audiofile.JavaSoundAudioFile");
        } catch (ClassNotFoundException e) {
            try {
                defaultAudioFileReaderClass = (Class<? extends AudioFileReader>) Class
                        .forName("net.beadsproject.beads.data.audiofile.WavFileReaderWriter");
    ...
    /*
     * Set the defaultAudioFileWriterClass to WavFileReaderWriter which should
     * always be available in beads_main.
     */
    static {
        try {
            defaultAudioFileWriterClass = (Class<? extends AudioFileWriter>) Class
                    .forName("net.beadsproject.beads.data.audiofile.JavaSoundAudioFile");
        } catch (ClassNotFoundException e) {
            try {
                defaultAudioFileWriterClass = (Class<? extends AudioFileWriter>) Class
    ...
    /**
     * Instantiates a new writable sample with specified length and default audio
     * format: 44.1KHz, float, stereo.
     * 
    ...
    /**
     * Instantiates a new writable sample with specified length and number of
     * channels and default audio format: 44.1KHz, float.
     * 
     * @param length    the length in ms.
     * 
     ...
     * length;
     * 
     * The sample isn't initialised, so may contain junk. Use {@link #clear()} to
     * clear it.
     * 
    ...
    /**
     * Create a sample from a file. This constructor immediately loads the entire
    ...
    /**
     * Gets the current AudioFileReaderClass. This is an instantiation of a class
     * that implements {@link AudioFileReader} that Sample will use for file reading
     * operations. If unset or set to null Sample will use a default.
     * 
     * @return
     */
    ...
    /**
     * Gets the current AudioFileWriterClass. This is an instantiation of a class
     * that implements {@link AudioFileWriter} that Sample will use for file writing
     * operations. If unset or set to null Sample will use a default.
     * 
     * @return
     */
    ...
    /**
     * Set the audioFileReaderClass. This is an instantiation of a class that
     * implements {@link AudioFileReader} that Sample will use for file reading
     * operations. If unset or set to null Sample use a default.
     * 
    ...
    /**
     * Set the audioFileWriterClass. This is an instantiation of a class that
     * implements {@link AudioFileWriter} that Sample will use for file writing
     * operations. If unset or set to null Sample use a default.
     * 
    ...
    /**
     * Return a single frame with the sample's bit-depth.
     * 
     ...
     * 
     */
    public void getFrameNoDecoding(int frame, float[] frameData) {
    ...
    /**
     * Return a single frame of 16 bit-depth.
     * 
     * If the data is not readily available this doesn't do anything to frameData.
     * 
     * @param frame     Must be in range, else framedata is unchanged.
    ...
    public void getFrame(int frame, float[] frameData) {
        ...
        for (int i = 0; i < nChannels; i++) {
            frameData[i] = encodeFrame(theSampleData[i][frame]);
            // frameData[i] = theSampleData[i][frame];
        }
    }

    /**
     * Retrieves a frame of audio using no interpolation. If the frame is not in the
     * sample range then zeros are returned. Bit-depth of 16.
     * 
    ...
    /**
     * Retrieves a frame of audio using linear interpolation. If the frame is not in
     * the sample range then zeros are returned. Bit-depth of 16.
     * 
    ...
    /**
     * Retrieves a frame of audio using cubic interpolation. If the frame is not in
     * the sample range then zeros are returned. Bit-depth of 16.
     * 
     * @param posInMS The frame to read -- can be fractional (e.g., 4.4).
     * @param result  The framedata to fill.
     ...
     * frames. It is the caller's responsibility to count how many frames are valid.
     * <code>min(nFrames - frame, frameData[0].length)</code> frames in frameData
     * are valid. Bit-depth of 16.
     * 
    ...
    public void getFrames(int frame, float[][] frameData) {
        ...
        for (int i = 0; i < nChannels; i++) {
            for (int n = 0; n < numFloats; i++) {
                frameData[i][n] = encodeFrame(theSampleData[i][frame]);
            }
        }
    }

    /**
     * When given an (raw) frame of sample, it reads the value and converts this
     * into a 16-bit depth sample for playback.
     * 
     * @param value The sample frame
     * @return a decoded 16-bit sample frame
     */
    private float encodeFrame(float value) {
        double floatOffset = (decodeValues == null) ? 0 : decodeValues[0];
        double floatScale = (decodeValues == null) ? 1 : decodeValues[1];
        return (float) (floatOffset + (double) value / floatScale);
    }

    /**
     * When given a 16-bit decoded frame, it converts this frame back into the bit
     * depth used when reading the sample.
     * 
     * @param value The encoded 16-bit frame
     * @return a decoded n-bit sample frame
     */
    private float decodeFrame(float value) {
        double floatOffset = (decodeValues == null) ? 0 : decodeValues[0];
        double floatScale = (decodeValues == null) ? 1 : decodeValues[1];
        return (float) (((double) value - floatOffset) * floatScale);
    }
    ...
    public void clear() {
        for (int i = 0; i < nChannels; i++) {
            Arrays.fill(theSampleData[i], 0);
        }

    }

    /**
     * Write a single frame of 16 bit-depth into this sample. Takes care of format
     * conversion.
     * 
     * This only makes sense if this.isWriteable() returns true. If isWriteable() is
     * false, the behaviour is undefined/unstable.
     * 
     * @param frame     The frame to write into. Must be greater than 0 and less
     *                  than numFrames.
     * @param frameData The frame data to write.
     */
    public void putFrame(int frame, float[] frameData) {
        for (int i = 0; i < nChannels; i++) {
            theSampleData[i][frame] = decodeFrame(frameData[i]);
        }
    }

    /**
     * Write a single frame into this sample, using the same bit-depth as the
     * sample.
     * 
     * This only makes sense if this.isWriteable() returns true. If isWriteable() is
     * false, the behaviour is undefined/unstable.
     ...
     * @param frameData The frame data to write.
     */
    public void putFrameNoEnconding(int frame, float[] frameData) {
        for (int i = 0; i < nChannels; i++) {
    ...
    /**
     * Write multiple frames of 16 bit-depth into the sample.
     * 
     * This only makes sense if this.isWriteable() returns true. If isWriteable() is
     * false, the behaviour is undefined/unstable.
     * 
    ...
    public void putFrames(int frame, float[][] frameData) {
        ...
        for (int i = 0; i < nChannels; i++) {
            ...
            for (int n = frame; n < numFrames; n++) {
                theSampleData[i][n] = decodeFrame(frameData[i][n]);
            }
        }
    }
    ...
    public void putFrames(int frame, float[][] frameData, int offset, int numFrames) {
        ...
        for (int i = 0; i < nChannels; i++) {
            System.arraycopy(frameData[i], offset, theSampleData[i], frame, numFrames);
            for (int n = frame; n < numFrames; n++) {
                theSampleData[i][frame] = decodeFrame(frameData[i][n]);
            }
        }
    }
    ...
    /**
     * This records the sample to a file with the specified AudioFile.Type. It is
     * BLOCKING.
     * 
    ...
    public void write(String fn, AudioFileType type, SampleAudioFormat saf) throws IOException {
        Class<? extends AudioFileWriter> theRealAudioFileWriterClass = audioFileWriterClass == null
                ? defaultAudioFileWriterClass
                : audioFileWriterClass;
        // JavaSound can only write 16-bit, but we can use WavFileReaderWriter for
        // >16-bit wavs, hence always write wavs this way
        if (type == AudioFileType.WAV) {
            try {
                theRealAudioFileWriterClass = (Class<? extends AudioFileWriter>) Class
                        .forName("net.beadsproject.beads.data.audiofile.WavFileReaderWriter");
            } catch (ClassNotFoundException e) {
     ...
     * <b>Advanced</b>
     * 
     * Change the number of frames in the (writeable) sample. This is slow and so
    ...
    private void loadAudioFile(String file) throws IOException, OperationUnsupportedException, FileFormatException {
        // we have to deal with a bug in Tritonus: JavaSound doesn't accept 24-bit wav
        // but strangely Tritonus
        // interprets 24-bit wavs as mp3s. So we intercept all wavs and send them to the
        // WavFileReaderWriter.
        // In the first instance we can only use the file suffix as a clue to this, not
        // the header.
        Class<? extends AudioFileReader> theRealAudioFileReaderClass = audioFileReaderClass == null
                ? defaultAudioFileReaderClass
        ...
        try {
            ...
            throw new IOException(
                    "Sample: No AudioFileReader Class has been set and the default JavaSoundAudioFile Class cannot be found. Aborting write(). You may need to link to beads-io.jar.");
        }
        try {
        ...
        this.decodeValues = audioFileReader.getEncodingValues();
    }
}
