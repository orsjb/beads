...
public class WavFileReaderWriter implements AudioFileReader, AudioFileWriter {
    ...
    private long sampleRate; // 4 bytes unsigned, 0x00000001 (1) to 0xFFFFFFFF (4,294,967,295); Although a
                             // java int is 4 bytes, it is signed, so need to use a long
    ...
    private int b = 0; // Local variable used for endian calculation
    ...
    /**
     * See
     * {@link net.beadsproject.beads.data.audiofile.AudioFileReader#getSampleAudioFormat}
     */
    ...
    private void writeHeader() throws IOException, FileFormatException {
        ...
        // Set the compression code: if 32 or 64 bits, automatically use floating point
        // format
    ...
    private void readHeader() throws IOException, FileFormatException, OperationUnsupportedException {
        ...
        if (validBits > 24) {
            // If more than 24 validBits, data is signed
            // Don't convert
            floatOffset = 0;
            floatScale = 1;          
        } else if (validBits > 8) {
    ...
    private byte[][] readByteData() throws IOException {
        ...
        byte[][] data = new byte[this.numChannels][(int) this.numFrames];
        long framesRead = 0;
        int offset = 0;
        int blockSize = 10000;
     ...
     * @param offset           position in buffer from where numFramesToWrite frames
     *                         will be retrieved for writing
     ...
     * @throws IOException
     */
    private int readFrames(byte[][] sampleBuffer, int offset, int numFramesToRead) throws IOException {
        ...
        for (int f = 0; f < numFramesToRead; f++) {
            ...
            for (int c = 0; c < numChannels; c++) {
                ...
                // floatScale);
                sampleBuffer[c][offset] = readByteSample();
            }
            offset++;
            frameCounter++;
        }
        ...
        if (compressionCode == WAVE_FORMAT_IEEE_FLOAT && this.validBits == 32) {
            ...
            for (int f = 0; f < numFramesToRead; f++) {
                ...
                for (int c = 0; c < numChannels; c++) {
                    // sampleBuffer[c][offset] = (float)(floatOffset + (double) readSample() /
                    // floatScale);
                    sampleBuffer[c][offset] = (float) readSample();
                }
                offset++;
                frameCounter++;
            }
        ...
        if (compressionCode == WAVE_FORMAT_IEEE_FLOAT && this.validBits == 32) {
            ...
            for (int f = 0; f < numFramesToRead; f++) {
                ...
                for (int c = 0; c < numChannels; c++) {
                    ...
                    sampleBuffer[c][offset] = (double) readSample();
                }
                offset++;
                frameCounter++;
            }
    ...
    /**
     * Read a single byte of sample
     * 
     * @return
     * @throws IOException
     */
    private byte readByteSample() throws IOException {
        byte val = 0;
    ...
    /**
     * Get the original bit depth of the sample.
     * 
     * @return the bit depth (bits per smaple) as an int.
     */
