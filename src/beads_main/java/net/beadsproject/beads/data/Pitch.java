...
public abstract class Pitch {
    ...
    /**
     * Takes a pitch and returns that pitch adjusted downwards to the nearest pitch
     * in the given scale.
     * 
    ...
    /**
     * Takes a pitch and returns that pitch adjusted downwards to the nearest pitch
     * in the given scale. Assumes 12 pitches per octave.
     * 
     * @param pitch the pitch to modify.
     * @param scale the scale to use.
     * @return adjusted pitch.
    ...
    public static final String[] pitchNames = new String[] { "C", "Db", "D", "Eb", "E", "F", "Gb", "G", "Ab", "A", "Bb",
            "B" };
    ...
    /**
     * Calculate the MIDI note that would be required given the tonic of a scale,
     * the numbers in the scale relative to the tonic, and the index using zero
     * based reference For example, with Midi Note C3 (60), using the Major scale,
     * and scaleIndex of 8 (being major 9th) we would get 60 + 12 + 2 = 74 (D4)
     * 
     ...
     * @return The scaleTonic + scaleIndex in the index. If scaleIndex is outside
     *         the range of array eg (scaleIndex &lt; 0 || scaleIndex &gt;=
     *         scaleData.length) then modulo mathematics will be used to return note
     *         from next or previous register
     */
    public static int getRelativeMidiNote(int scaleTonic, int[] scaleData, int scaleIndex) {
        ...
        // Note our comment text will assume scaleData is Picth.major for ease of
        // understanding
        if (scaleIndex > 0) {
            ...
            // add the number to our base tonic to get the note based on key. Assume our
            // scaleTonic is C2 (48)
            ...
            // eg, if scaleIndex is -9, scale_degree = 9 % 7 = 2 (we got absoly=te value of
            // scaleIndex)
            ...
            // if it is not exactly an octave, we need to calculate our range from and extra
            // register below
            if (scale_pitch != 0) {
