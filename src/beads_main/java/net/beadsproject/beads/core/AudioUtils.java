 ...
 * different formats, interleaving, and some other miscellaneous audio
 * functions.
 * 
 * @author ollie
 */
public final class AudioUtils {
     ...
     */
    static final public void floatToByte(byte[] out, int outstart, float[] in, int instart, int inlength,
            boolean bigEndian) {
        int bufsz = Math.min(inlength, in.length);
        int ib = outstart;
        if (bigEndian) {
            for (int i = 0; i < bufsz; ++i) {
                short y = (short) (32767. * Math.min(Math.max(in[i + instart], -1.0f), 1.0f));
     ...
     * @param numFloats             number of elements to copy into out
     */
    static final public void byteToFloat(float[] out, byte[] in, boolean bigEndian, int startIndexInByteArray,
            int numFloats) {
     ...
     * @param out                    buffer of floats.
     * @param in                     buffer of bytes.
     * @param bigEndian              true for big endian byte order, false
     *                               otherwise.
     * @param startIndexInByteArray  where to start copying from
     ...
     */
    static final public void byteToFloat(float[] out, byte[] in, boolean bigEndian, int startIndexInByteArray,
            int startIndexInFloatArray, int numFloats) {
        if (bigEndian) {
     ...
     * @param out                    buffer of floats.
     * @param in                     buffer of bytes.
     * @param nBytesPerFloat         number of bytes per float, maximum of 4
     * @param bigEndian              true for big endian byte order, false
     *                               otherwise.
     * @param startIndexInByteArray  where to start copying from
     * @param startIndexInFloatArray where to start copying to
     ...
     */
    /*
     * static final public void byteToFloatWithNBytesPerFloat(float[] out, byte[]
     * in, int nBytesPerFloat, boolean bigEndian, int startIndexInByteArray, int
     * startIndexInFloatArray, int numFloats) { float maxvalue = (float)(1 <<
     * (nBytesPerFloat*8)); System.out.println(maxvalue);
     * 
     * if (bigEndian) { int ib = startIndexInByteArray; int min =
     * Math.min(out.length,startIndexInFloatArray+numFloats); for (int i =
     * startIndexInFloatArray; i < min; ++i) { // float sample = ((in[ib + 0] << 8)
     * | (in[ib + 1] & 0xFF)) / maxvalue;
     * 
     * long l = 0; for(int k=0;k<nBytesPerFloat;k++) l |= (in[ib+k]&0xFF) <<
     * (8*((nBytesPerFloat-1)-k)); float sample = l / maxvalue; ib +=
     * nBytesPerFloat; out[i] = sample; } } else { int ib = startIndexInByteArray;
     * int min = Math.min(out.length,startIndexInFloatArray+numFloats); for (int i =
     * startIndexInFloatArray; i < min; ++i) { // float sample = ((in[ib] & 0xFF) |
     * (in[ib + 1] << 8)) / maxvalue;
     * 
     * long l = 0; for(int k=0;k<nBytesPerFloat;k++) l |= (in[ib+k]&0xFF) << (8*k);
     * float sample = l / maxvalue;
     * 
     * ib += nBytesPerFloat; out[i] = sample; } } }
     */

    /**
     * De-interleave an interleaved buffer of floats to form a 2D array of floats of
     * size nChannels x nFrames.
     * 
    ...
    /**
     * Interleave a 2D array of floats of size nChannels x nFrames to form a single
     ...
     * @param result    the result
     */
    static final public void interleave(float[][] source, int nChannels, int nFrames, float[] result) {
     ...
     * @param result    the result
     */
    static final public void interleave(float[][] source, int nChannels, int nFrames, int offset, float[] result) {
    ...
    static final public void stretchBuffer(float[][] source, float[][] dest) {
        ...
        for (int ch = 0; ch < numChannels; ch++) {
            ...
            // each segment of src is considered a linear region that is interpolated to map
            // onto dst,
            // for each segment, only need to calculate the gradient once
            // the first and last elements of src are mapped into the first and last
            // elements of dst
            dst[0] = src[0];
            dst[dst.length - 1] = src[src.length - 1];

            /*
             * algorithm: step in dst foreach element of dst figure out whether we are in a
             * new segment of src if so, recalculate the lerp data for that segment
             */
    ...
    /**
     * Does a freaky shortcut for calculating pow (limited to base with range 0-1),
     * faster but less accurate than regular Math.pow().
     * 
     * CREDIT: this method is copied directly from <a href=
     * "http://martin.ankerl.com/2007/10/04/optimized-pow-approximation-for-java-and-c-c/">http://martin.ankerl.com/2007/10/04/optimized-pow-approximation-for-java-and-c-c/</a>
     * 
     * @param a the base.
    ...
    /**
     * Attempts to determine a URL given a String. Firstly the String is interpreted
     * as a System Resource. Failing that, it is interpreted as a proper URL.
     * Failing that it is interpreted as a file path. All Exceptions are suppressed
    ...
    /**
     * Attempts to get a File from a URL, suppressing warnings. Assumes UTF-8
     * encoding.
     * 
