 ...
 * Its purpose is to forward messages to its array members. A BeadArray detects
 * whether or not its members are deleted, and removes them if they are. For
 * this reason it should be used in any situations where a Bead needs to be
 * automatically disposed of. Note, however, that a BeadArray does not forward
 * {@link Bead#kill()}, {@link Bead#start()} and {@link Bead#pause(boolean)}
 * messages to its component Beads unless told to do so by setting
 * {@link #setForwardKillCommand(boolean)} and
 * {@link BeadArray#setForwardPauseCommand(boolean)} respectively.
 * 
 * @author ollie
 */
