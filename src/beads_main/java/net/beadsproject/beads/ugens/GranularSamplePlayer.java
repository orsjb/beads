...
/**
 * GranularSamplePlayer plays back a {@link Sample} using granular synthesis.
 * GranularSamplePlayer inherits its main behaviour from {@link SamplePlayer}
 * but replaces the direct {@link Sample} lookup with a granular process.
 * {@link UGen}s can be used to control playback rate, pitch, loop points, grain
 * size, grain interval, grain randomness and position (this last case assumes
 * that the playback rate is zero).
 * 
...
public class GranularSamplePlayer extends SamplePlayer {
    ...
    private void resetGrain(Grain g, int time) {
        g.position = position + (grainSizeEnvelope.getValue(0, time) * randomnessEnvelope.getValue(0, time)
                * (Math.random() * 2.0 - 1.0));
        g.age = 0f;
