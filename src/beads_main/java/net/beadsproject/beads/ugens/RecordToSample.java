 ...
 * 
 * RecordToSample has three different modes, which dictate how it behaves when
 ...
 * </ul>
 *
 * A recorder may not completely fill a sample. If you just want the recorded
 * data then be sure to {@link #clip()} the sample once done. Alternatively you
 * can see {@link #getNumFramesRecorded() how many frames were written}.
 * 
 * <p>
 * <b>Tip: </b> Be sure to {@link #pause(boolean)} the recorder when using
 * INFINITE mode, otherwise it will keep recording and you may quickly run out
 * of memory.
 * </p>
 * 
 * <i>ADVANCED:</i> When resizing a sample in INFINITE mode, the recorder uses a
 * set of parameters that specify how it behaves. If necessary you can modify
 * the parameters on a per-recorder basis. See
...
public class RecordToSample extends UGen {
     ...
     * 
     * A recorder may never write the entire length of the sample. Hence this
     * variable keeps track of the recorded section.
     * 
     * In particular, in INFINITE mode, framesWritten will always be less than
     * sample.getNumFrames(). In this case, it is often necessary to resize the
     * sample once you have finished writing into it. This is where clip() comes in.
     **/
    private long framesWritten;

    public enum Mode {
        FINITE, LOOPING, INFINITE
    };
     ...
     * @param sample  the Sample. //@throws Exception if sample is not writeable.
     */
     ...
     * @param mode    the Recording Mode to use. //@throws Exception if sample is
     *                not writeable.
     */
     ...
     *
     * @param sample the Sample.
     * @param mode   the Recording Mode to use. //@throws Exception if sample is not
     *               writeable.
     */
        ...
        if (sample != null) {
            ...
            if ((position + bufferSize) >= nFrames) {
                ...
                case LOOPING: {
                    ...
                    /*
                     * nFrames V [ sample ] [ bufIn ] A<--bs-> | pos
                     * 
                     * first chunk [f2e] second chunk [ ]
                     */
    ...
    /**
     * <i>Advanced:</i> Change the parameters used when resizing samples in INFINITE
     * recorder mode.
     * 
     * In INFINITE mode the recorder resizes the sample when it needs to write more
     * data into it. Initially the recorder doubles the length of the sample, up to
     * a particular length. Once the sample size is past this length the recorder
     * stops doubling the length, and simply resizes the sample by a constant
     * amount.
     * 
     * @param doubleUpTime         The time (in ms) up to which the sample size
     *                             should be doubled.
     * @param constantResizeLength The length (in ms) of the extra space appended to
     *                             Sample.
     */
     ...
     * @deprecated Use {@link #getMode()} instead. //@Deprecated Use
     *             {@link #getMode()} instead.
     */
     ...
     * @deprecated Use {@link #setMode(Mode)} instead. //@Deprecated Use
     *             {@link #setMode(Mode)} instead.
     */
