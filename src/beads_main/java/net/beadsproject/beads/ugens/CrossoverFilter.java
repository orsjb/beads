...
public class CrossoverFilter extends UGen implements DataBeadReceiver {
    ...
    /**
     * Constructor for a crossover filter with the specified number of channels, set
        ...
        if (channels > 1) {
            for (int chan = 0; chan < channels; chan++) {
                ...
                for (int i = 0; i < bufferSize; i++) {
                    ...
                    ly[0] = (lb0 * (bi[i] + x[2]) + lb1 * x[1] - a1 * ly[1] - a2 * ly[2]) / a0;
                    lo[i] = (lb0 * (ly[0] + ly[2]) + lb1 * ly[1] - a1 * lz[1] - a2 * lz[2]) / a0;

                    // optimized high-pass part - two identical butterworth
                    // filters
                    hy[0] = (bi[i] - 2f * x[1] + x[2] - a1 * hy[1] - a2 * hy[2]) / a0;
                    hi[i] = (hy[0] - 2f * hy[1] + hy[2] - a1 * hz[1] - a2 * hz[2]) / a0;
            ...
            for (int i = 0; i < bufferSize; i++) {
                ...
                // low-pass part - two identical butterworth filters
                ly0 = (lb0 * (bi[i] + x2) + lb1 * x1 - a1 * ly1 - a2 * ly2) / a0;
                lo[i] = (lb0 * (ly0 + ly2) + lb1 * ly1 - a1 * lz1 - a2 * lz2) / a0;
    ...
    /**
     * Sets a UGen to control the cutoff frequency. Note that the frequency is only
     * updated every frame, not every sample.
     * 
    ...
    /**
     * Sets the crossover frequency with a DataBead. If the value of the property
     * "frequency" is either a UGen or can be interpreted as a float, the cutoff
     * frequency will be set appropriately.
     */
     ...
     * 
     * for (int i = 0; i < bufferSize; i++) { lo[i] = (la0 * bi[i] + la1 * x1 + la2
     * * x2 + la3 * x3 + la4 * x4 - b1 * ly1 - b2 * ly2 - b3 * ly3 - b4 * ly4) / b0;
     * hi[i] = (ha0 * bi[i] + ha1 * x1 + ha2 * x2 + ha3 * x3 + ha4 * x4 - b1 * hy1 -
     * b2 * hy2 - b3 * hy3 - b4 * hy4) / b0; x4 = x3; x3 = x2; x2 = x1; x1 = bi[i];
     * 
     ...
     * 
     * b0 = 4 + nr2k + nik4 + nr2k3 + ok2; b1 = 4 * (ok2 + nr2k - nik4 - nr2k3); b2
     * = (6 * (ok2 + nik4)) - 8; b3 = 4 * (ok2 - nr2k + nr2k3 - nik4); b4 = nik4 -
     * (2 * (nr2k + nr2k3)) + ok2 + 4;
     * 
     * // ================================================ // low-pass //
     * ================================================ la4 = la0 = ok2; la3 = la1 =
