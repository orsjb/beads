...
public class LPRezFilter extends IIRFilter implements DataBeadReceiver {
    ...
    /**
     * Constructor for a multi-channel LPRezFilter with default values for frequency
     * and resonance.
     * 
     * @param con      The audio context.
    ...
    /**
     * Constructor for a single-channel LPRezFilter with frequency specified by a
    ...
    public void calculateBuffer() {
        ...
        if (channels == 1) {
            ...
            if (isFreqStatic && isResStatic) {
                ...
                cosw = (float) (Math.cos(_2pi_over_sr * (freq = freqUGen.getValue(0, 0))));
                ...
                cosw = (float) (Math.cos(_2pi_over_sr * (freq = freqUGen.getValue(0, 1))));
                ...
                for (int currsamp = 2; currsamp < bufferSize; currsamp++) {
                    ...
                    cosw = (float) (Math.cos(_2pi_over_sr * (freq = freqUGen.getValue(0, currsamp))));
            ...
            if (isFreqStatic && isResStatic) {
                ...
                calcVals();
                for (int i = 0; i < channels; i++) {
                    bufOut[i][1] = bufIn[i][1] * b0 - a1 * bufOut[i][0] - a2 * y1m[i];
                }
                ...
                for (int currsamp = 2; currsamp < bufferSize; currsamp++) {
                    ...
                    for (int i = 0; i < channels; i++) {
                        bufOut[i][currsamp] = bufIn[i][currsamp] * b0 - a1 * bufOut[i][currsamp - 1]
                                - a2 * bufOut[i][currsamp - 2];
                    }
                }
    ...
    /**
     * Sets the cut-off frequency to a float. Removes the frequency UGen, if there
     * is one.
     * 
    ...
    /**
     * Sets a UGen to specify the cut-off frequency. Passing a null value freezes
     * the parameter.
     * 
    ...
    /**
     * Sets the filter resonance to a float value. This removes the resonance UGen,
     * if it exists. (Should be &gt;= 0 and &lt; 1.)
     * 
    ...
    /**
     * Sets a UGen to specify the filter resonance. Passing a null value freezes the
     * parameter.
     * 
