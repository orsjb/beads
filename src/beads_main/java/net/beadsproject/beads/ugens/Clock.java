...
/**
 * A sample rate Clock. A Clock generates timing data at two levels: ticks and
 * beats. It notifies an {@link BeadArray} of listeners at each tick. These
 * listeners can query the Clock to find out the current tick count, if it is on
 * a beat, and the current beat count. The rate of ticking of the Clock is
 * controlled by an interval envelope.
 *
 * @author ollie
 */
public class Clock extends UGen implements IntegerBead {
    ...
    /**
     * Used so that other objects can discover what the tick times were from the
     * last buffer update.
     */
    ...
    /**
     * Deprecated. Use {getIntervalUGen()} instead. Gets the interval envelope.
     * 
     ...
     * 
     * @return true if the clock is on a beat, and the current beat count is a
     *         multiple of mod.
     */
