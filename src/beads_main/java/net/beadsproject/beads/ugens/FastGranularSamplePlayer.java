...
/**
 * FastGranularSamplePlayer functions exactly the same as
 * {@link GranularSamplePlayer} but trades off variable automation functionality
 * to reduce computational costs and allow for more instances to be run
 * simultaneously. As with GranularSamplePlayer, it inherits its main behaviour
 * from {@link SamplePlayer} but replaces the direct {@link Sample} lookup with
 * a granular process. {@link UGen}s can be used to control playback rate,
 * pitch, loop points, grain size, grain interval, grain randomness and position
...
public class FastGranularSamplePlayer extends SamplePlayer {
    ...
    /**
     * The nested class Grain. Stores information about the start time, current
     * position, age, and grain size of the grain.
     */
    ...
    /**
     * Sets the rate to the UGen's current value. For better performance, use
     * {@link #setRate(float)}
     * 
    ...
    /**
     * Sets the loop end to the UGen's value. For better performance, use
     * {@link #setLoopEnd(float)} instead.
     * 
     ...
     * Sets the loop end.
     * 
     * @param loopEnd the new loop end.
     */
    ...
    /**
     * Sets the loop start as the UGen's value. For better performance, use
     * {@link #setLoopStart(float)} instead.
     * 
     ...
     * Sets the loop start.
     * 
     * @param loopStart the new loop start.
     */
    ...
    /**
     * Sets the grain interval as the UGen's value. For better performance, use
     * {@link #setGrainInterval(float)} instead.
     * 
     ...
     * 
     * @param grainInterval the new grain interval.
     */
    ...
    /**
     * Sets the grain size as the UGen's value. For better performance, use
     * {@link #setGrainSize(float)} instead.
     * 
    ...
    /**
     * Sets the randomness as the UGen's value. For better performance, use
     * {@link #setRandomness(float)} instead.
     * 
     ...
     * 
     * @see
     * net.beadsproject.beads.ugens.SamplePlayer#setBuffer(net.beadsproject.beads.
     * data.Sample)
     */
    ...
    private void calculateNextGrainPosition(Grain g) {
        int direction = rate >= 0 ? 1 : -1; // this is a bit odd in the case when controlling grain from
                                            // positionEnvelope
        g.age += msPerSample;
        if (loopInsideGrains) {
            switch (loopType) {
    ...
    /**
     * Calculates the average number of Grains given the current grain size and
     * grain interval.
     * 
