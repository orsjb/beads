...
/**
 * CrossFade is used to quickly crossfade between elements. Add it to the signal
 * chain and simply call {@link #fadeTo(UGen, float)} to fade to a new
 * {@link UGen} over a given duration. Note that UGen has it's own simpler
 * crossFade method.
 * 
...
public class CrossFade extends UGen {
    ...
    /**
     * Create a new CrossFade with given number of channels. The CrossFade has no
     * inputs since the input UGens are assigned in the method
     * {@link #fadeTo(UGen, float)}.
     * 
    ...
    /**
     * Cross fades from the current UGen to the specified UGen the specified number
     * of milliseconds.
     * 
    ...
    /**
     * True if this CrossFade is set to pause outgoing UGens once they have been
     * faded out.
     * 
    ...
    /**
     * Set whether this CrossFade is set to pause outgoing UGens once they have been
     * faded out.
     * 
     * @param pauseAfterComplete true means that outgoing UGens will be paused once
     *                           they have been faded out.
     */
    ...
    public void calculateBuffer() {
        ...
        for (int j = 0; j < bufferSize; j++) {
            if (currentTimeSamps >= crossfadeTimeSamps) {
                ...
                for (int i = 0; i < outs; i++) {
                    bufOut[i][j] = incomingLevel * incoming.getValue(i, j) + outgoingLevel * outgoing.getValue(i, j);
                }
                currentTimeSamps++;
            }
        }
