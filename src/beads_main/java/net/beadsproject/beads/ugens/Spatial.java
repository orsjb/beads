...
/**
 * A mixer for a speaker config in any number of dimensions (we haven't tested D
 * &gt; 3 yet). Add sources (UGens) and control their locations using other
 * UGens. Locations are changed on a per-channel basis, so that multichannel
 * files can be located in the mixer independently.
 * 
 ...
 * 
 * first layout speakers 1-4 on the ground in clockwise order (2D and 3D) then
 * layout speakers 5-8 so that they are above 1-4 respectively. Then according
 * to the 'right-handed' ordering: the x-axis follows the line joining 1 and 4
 * the y-axis follows the line joining 1 and 2 the z-axis follows the line
 * joining 1 and 5
 *
 * @author ollie
 *
...
public class Spatial extends UGen {
    ...
    /**
     * The Class Location. Stores a source UGen and a set of UGens that control the
     * position of each channel of that UGen along each dimension.
     */
        ...
        void mixInAudio(float[][] output) {
            ...
            for (int outputChannel = 0; outputChannel < pos.length; outputChannel++) {
                ...
                for (int time = 0; time < bufferSize; time++) {
                    // get current position of this channel using
                    // pos[outputChannel][dim].getValue(0, time);
                    ...
                    for (int speaker = 0; speaker < speakerPositions.length; speaker++) {
                        ...
                        /*
                         * TODO I've removed the math pow because it was really slowing things down. the
                         * fastPow01 method doesn't help much either. Surprising this should make such a
                         * great difference.
                         */
...
//						speakerGains[speaker] = (float)AudioUtils.fastPow01(linearGain, curve);
    ...
    /*
     * Default speaker numbering: first layout speakers 1-4 on the ground in
     * clockwise order then layout speakers 5-8 so that they are above 1-4
     * respectively. Then: the x-axis follows the line joining 1 and 4 the y-axis
     * follows the line joining 1 and 2 the z-axis follows the line joining 1 and 5
     * This follows the 'right-handed' ordering of the axes:
     * http://en.wikipedia.org/wiki/Cartesian_coordinate_system#
     * Orientation_and_handedness
     * 
     */
    /** The dimensions. */
    ...
    /**
     * Instantiates a new Spatial with given AudioContext and dimensions. The
     * default speaker config for the dimensionality is used, and the default
     * sphereDiameter (equal to Math.sqrt(dimensions)).
     * 
    ...
    public Spatial(AudioContext context, int dimensions, float sphereDiameter) {
        ...
        switch (dimensions) {
        case 1:
            setSpeakerPositions(new float[][] { { 0 }, { 1 } });
            break;
        case 2:
            setSpeakerPositions(new float[][] { { 0, 0 }, { 0, 1 }, { 1, 1 }, { 1, 0 } });
            break;
        case 3:
            setSpeakerPositions(new float[][] { { 0, 0, 0 }, { 0, 1, 0 }, { 1, 1, 0 }, { 1, 0, 0 }, { 0, 0, 1 },
                    { 0, 1, 1 }, { 1, 1, 1 }, { 1, 0, 1 } });
            break;
        default:
            new IllegalArgumentException("Error, that's a stupid number of dimensions: " + dimensions + "!")
                    .printStackTrace();
        }
     ...
     * The locations array is an array of the form float[speakerNumber][dimension].
     *
     * @param context    the context.
     * @param dimensions the dimensions.
    ...
    /**
     * Instantiates a new Spatial with the given AudioContext, dimensions, locations
     * and sphereDiamater. The locations array is an array of the form
     * float[speakerNumber][dimension].
     *
     * @param context        the context
     * @param dimensions     the dimensions
     * @param locations      the locations
    ...
    /**
     * Sets the sphere diameter. This is the distance beyond which sound is
     * attenuated to zero.
     * 
    ...
    /**
     * Gets speaker positions from file. File data must correspond to given
     * dimensions.
     * 
     * @param file       the file.
     * @param dimensions the dimensions.
    ...
    /**
     * Sets the speaker positions. The locations array is an array of the form
     * float[speakerNumber][dimension].
     * 
    ...
    public void setSpeakerPositions(float[][] locations) {
        if (locations.length > 0 && locations[0].length != dimensions) {
            new IllegalArgumentException("Error, location data does not correspond to dimensions: " + dimensions + "!")
                    .printStackTrace();
            return;
        }
    ...
    /**
     * This overrides {@link #addInput(UGen)} by adding a new 'source' sound to the
     * spatialisation.
     */
    ...
    /**
     * Adds a new source sound with the given UGen controllers for controlling its
     * position. The array of controllers is of the form UGen[channel][dimension]
     * where channel is the output channel of the source UGen, treated as a point
     * source in the space and dimension is the axis along which the controller UGen
     * is controlling position.
     * 
     * @param source      the source.
     * @param controllers the controllers.
     */
    ...
    /**
     * Sets the location of a UGen at the give channel with glide. This assumes the
     * UGen's position is not being controlled by other external UGens.
     * 
     * @param source  the source
     * @param channel the channel
     * @param newPos  the new pos
...
//	}
     ...
     * 
     * @see
     * net.beadsproject.beads.core.UGen#removeAllConnections(net.beadsproject.beads.
     * core.UGen)
     */
    @Override
    ...
    /**
     * Sets the curve. The curve defines the attenuation from each speaker from 1
     * when the source is next to the speaker to zero when the source is at
     * sphereDiamater away from the speaker or further. The curve defines the
     * exponent of the attenuation, such that numbers greater than 1 drop more
     * rapidly near the speaker whilst numbers less than 1 (greater than zero
     * please) drop more gently.
     * 
