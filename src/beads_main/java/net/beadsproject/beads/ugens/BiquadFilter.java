...
public class BiquadFilter extends IIRFilter implements DataBeadReceiver {
    ...
    /**
     * Indicates a low-pass filter; coefficients are calculated from equations given
     * in "Cookbook formulae for audio EQ biquad filter coefficients" by Robert
     * Bristow-Johnson.
     */
     ...
     * equations given in "Cookbook formulae for audio EQ biquad filter
     ...
     * {@link #setCustomType(CustomCoeffCalculator) setCustomType}. This constant is
     * not recognized by {@link #setType(int) setType}.
     */
    public final static Type CUSTOM_FILTER = Type.CUSTOM_FILTER;

    public enum Type {
        LP, HP, BP_PEAK, BP_SKIRT, NOTCH, AP, PEAKING_EQ, LOW_SHELF, HIGH_SHELF, BUTTERWORTH_LP, BUTTERWORTH_HP,
        BESSEL_LP, BESSEL_HP, CUSTOM_FILTER
    }
    ...
    /**
     * Constructor for a multi-channel biquad filter UGen of specified type with the
     * specified number of channels. See {@link #setType(int) setType} for a list of
    ...
    /**
     * Constructor for a multi-channel biquad filter UGen with the specified number
     * of channels and parameters specified by a DataBead.
     * 
     * @param context  The audio context.
     * @param channels The number of channels.
     ...
     * the specified number of channels, and with parameters specified by a
     * DataBead.
     * 
     * @param context  The audio context.
     * @param channels The number of channels.
     ...
     *                 {@link #setParams(DataBead)}.
     */
    public BiquadFilter(int channels, Type itype, DataBead params) {

        this(getDefaultContext(), channels, itype, params);
    }

    /**
     * Constructor for frequency and Q as floats. See {@link #setType(int) setType}
     ...
     * @param iqval The initial Q-value.
     */
    public BiquadFilter(Type itype, float ifreq, float iqval) {
    ...
    /**
     * Constructor for frequency and Q as UGens. See {@link #setType(int) setType}
     * for a list of supported filter types.
     * 
     * @param context The AudioContext.
     * @param itype   The initial filter type, e.g. {@link #LP}, {@link #HP},
    ...
    public void calculateBuffer() {
        ...
        if (channels == 1) {
            ...
            if (areAllStatic) {
                ...
                bo[1] = (b0 * bi[1] + b1 * bi[0] + b2 * bi1 - a1 * bo[0] - a2 * bo1) / a0;

                // main loop
                for (int currsamp = 2; currsamp < bufferSize; currsamp++) {
                    bo[currsamp] = (b0 * bi[currsamp] + b1 * bi[currsamp - 1] + b2 * bi[currsamp - 2]
                            - a1 * bo[currsamp - 1] - a2 * bo[currsamp - 2]) / a0;
                }
            ...
            if (areAllStatic) {
                ...
                for (int i = 0; i < channels; i++) {
                    ...
                    // first two samples
                    bo[0] = (b0 * bi[0] + b1 * bi1m[i] + b2 * bi2m[i] - a1 * bo1m[i] - a2 * bo2m[i]) / a0;
                    bo[1] = (b0 * bi[1] + b1 * bi[0] + b2 * bi1m[i] - a1 * bo[0] - a2 * bo1m[i]) / a0;
                ...
                vc.calcVals();
                for (int i = 0; i < channels; i++) {
                    bufOut[i][1] = (b0 * bufIn[i][1] + b1 * bufIn[i][0] + b2 * bi1m[i] - a1 * bufOut[i][0]
                            - a2 * bo1m[i]) / a0;
                }
                ...
                for (int currsamp = 2; currsamp < bufferSize; currsamp++) {
                    ...
                    for (int i = 0; i < channels; i++) {
                        bufOut[i][currsamp] = (b0 * bufIn[i][currsamp] + b1 * bufIn[i][currsamp - 1]
                                + b2 * bufIn[i][currsamp - 2] - a1 * bufOut[i][currsamp - 1]
                                - a2 * bufOut[i][currsamp - 2]) / a0;
                    }
    ...
    private class PeakingEQValCalculator extends ValCalculator {
        public void calcVals() {
             ...
             * 
             * b0 = 1 + alpha*A b1 = -2*cos(w0) b2 = 1 - alpha*A a0 = 1 + alpha/A a1 =
             * -2*cos(w0) a2 = 1 - alpha/A
             */
        }
    }
    ...
    private class LowShelfValCalculator extends ValCalculator {
        public void calcVals() {
             ...
             * 
             * b0 = A*( (A+1) - (A-1)*cos(w0) + 2*sqrt(A)*alpha ) b1 = 2*A*( (A-1) -
             * (A+1)*cos(w0) ) b2 = A*( (A+1) - (A-1)*cos(w0) - 2*sqrt(A)*alpha ) a0 = (A+1)
             * + (A-1)*cos(w0) + 2*sqrt(A)*alpha a1 = -2*( (A-1) + (A+1)*cos(w0) ) a2 =
    ...
    private class HighShelfValCalculator extends ValCalculator {
        public void calcVals() {
             ...
             * b0 = A*( (A+1) + (A-1)*cos(w0) + 2*sqrt(A)*alpha ) b1 = * -2*A*((A-1) +
             * (A+1)*cos(w0) ) b2 = A*( (A+1) + (A-1)*cos(w0) - 2*sqrt(A)*alpha ) a0 = (A+1)
             * - (A-1)*cos(w0) + 2*sqrt(A)*alpha a1 = 2*( (A-1) - (A+1)*cos(w0) ) a2 = (A+1)
             * - (A-1)*cos(w0) - 2*sqrt(A)*alpha
             */
        }
    }
    ...
    /**
     * Gets a DataBead with the filter's parameters (whether float or UGen), stored
     * in the keys "frequency", "q", "gain", and "filterType".
     * 
     ...
     * {@link #setCustomType(CustomCoeffCalculator) setCustomType}. The following
    ...
    /**
     * Sets the type of filter with an integer. This method is deprecated and has
     * been kept for backwards-compatibility reasons only. { {@link #setType(Type)}
     * should be used.
     * <ul>
    ...
    /**
     * Sets the filter frequency to a float value. This will remove the frequency
     * UGen, if there is one.
     * 
    ...
    /**
     * Sets the filter Q-value to a float. This will remove the Q UGen if there is
     * one.
     * 
    ...
    /**
     * Sets the filter gain to a float. This will remove the gain UGen if there is
     * one. (Only relevant for {@link #PEAKING_EQ}, {@link #LOW_SHELF}, and
     ...
     * response (real), frequency response (imaginary), amplitude response, phase
     * response, phase delay, group delay}.
     * 
     ...
     * defined in a user-defined class that extends {@link CustomCoeffCalculator}.
     * 
     ...
     * coefficients for a biquad filter based on frequency and Q. Users can create
     * their own coefficient calculator classes by extending this class and passing
     * it to a BiquadFilterMulti instance with
