 ...
 * <li>Limits the result between <code>-limit</code> and
 * <code>limit</code>.</li>
...
public class WaveShaper extends UGen implements DataBeadReceiver {
    ...
    protected boolean isPreGainStatic = false, isPostGainStatic = false, isLimitStatic = false, isWetMixStatic = false;
    ...
    /**
     * Constructor for a mono-channel wave shaper that uses a default cosine-based
     * wave shape.
     * 
    ...
    /**
     * Constructor for a multi-channel wave shaper that uses a default cosine-based
     * wave shape.
     * 
     * @param context  The audio context.
    ...
    /**
     * Constructor for a mono-channel wave shaper that uses the provided float array
     * for its wave shape.
     * 
     * @param context The audio context.
    ...
    /**
     * Constructor for a mono-channel wave shaperthat uses the float array from a
     * Buffer for its wave shape.
     * 
     * @param context     The audio context.
     * @param shapeBuffer The Buffer from which to get the wave shape.
     */
    ...
    /**
     * Constructor for a multi-channel wave shaper that uses the float array from a
     * Buffer for its wave shape.
     * 
     * @param context     The audio context.
     * @param channels    The number of channels.
    ...
    /**
     * Generates a nice cosine-based waveform in a float array that will provide a
     * little warmth when used for wave-shaping.
     * 
    ...
    /**
     * Generates an exponentially-based waveform in a float array. Negative input
    ...
    public void calculateBuffer() {
        ...
        if (channels == 1) {
            ...
            for (int currsample = 0; currsample < bufferSize; currsample++) {
                ...
                float y, y2;
                float y1 = (((y = bi[currsample]) * preGain * .5f) + .5f) * shapeLen;
                ...
                if (y1 <= 0) {
                    ...
                    int ind = (int) y1;
                    float frac = y1 - ind;
                    y2 = (shape[ind] * (1 - frac) + shape[ind + 1] * frac) * postGain;
                }
            ...
            for (int currsample = 0; currsample < bufferSize; currsample++) {
                ...
                for (int currchannel = 0; currchannel < channels; currchannel++) {
                    float y1 = (((y = bufIn[currchannel][currsample]) * preGain * .5f) + .5f) * shapeLen;
                    ...
                    bufOut[currchannel][currsample] = y * (1 - wetMix) + y2 * wetMix;
                }
            }
        }
     ...
     * corresponding parameters accordingly. Each property (except "shape", which
     * should be a float array) can be specified by either a float or a controller
     * UGen.
     */
    ...
    /**
     * Gets a new DataBead filled with the various parameter values stored in the
     * properties "preGain", "postGain", "limit", "wetMix", and "shape". Stores
     * UGens if the parameter is not static.
     * 
     ...
     * Gets a new DataBead filled with the various parameter values stored in the
     * properties "preGain", "postGain", "limit", "wetMix", and "shape". Stores
     * current values as floats only, even if the parameter is controlled by a UGen.
     * 
     * @return The new DataBead.
     */
