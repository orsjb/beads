...
public class Reverb extends UGenChain implements DataBeadReceiver {
    ...
    public Reverb(AudioContext context, int outChannels) {
        ...
        eAPF1 = new AllpassFilter(context, (int) (12.812 * sampsPerMS), 113, .3f);
        eAPF2 = new AllpassFilter(context, (int) (12.812 * sampsPerMS * 3), 337, .4f);
        eAPF3 = new AllpassFilter(context, (int) (12.812 * sampsPerMS * 9.4), 1051, .5f);
        ...
        delayModulator = new RandomPWM(context, RandomPWM.RAMPED_NOISE, 4000, 15000, 1);
        ...
        for (int i = 0; i < outChannels; i++) {
            float g = .3f + ((float) i / (i + 1)) * .1f + (float) Math.sin(i) * .05f;
            outDelayScale[i] = (3f * i + 5) / (5f * i + 5);
            apfOuts[i] = new AllpassFilter(context, (int) (60f * sampsPerMS), 20, g);
            apfOuts[i].addInput(collectedGain);
            addToChainOutput(i, apfOuts[i]);
        }

        setSize(.5f).setDamping(.7f).setEarlyReflectionsLevel(1).setLateReverbLevel(1);
    }
    ...
    /**
     * Sets the "room size". Valid value range from 0 to 1 (.5 is the default). The
     * larger the value, the longer the decay time.
     * 
    ...
    /**
     * Sets the damping factor. Valid values range from 0 to 1 (.7 is the default).
     * Higher values filter out higher frequencies faster.
     * 
    ...
    /**
     * Sets the early reflections level (the amount of early reflections heard in
     * the output). The default value is 1.
     * 
    ...
    /**
     * Sets the reverb parameters with a DataBead, using values stored in the keys
    ...
    public DataBeadReceiver sendData(DataBead db) {
        if (db != null) {
            ...
            setEarlyReflectionsLevel(db.getFloat("earlyReflectionsLevel", earlyLevel));
