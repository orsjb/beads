...
/**
 * A convenience class that wraps a Gain/Envelope combo to make an ADSR that
...
public class ADSR extends UGen {
    ...
    /**
     * Create an ADSR with the given {@link UGen} as input. The array of float
     * arguments should consist of a series of pairs [x,t] where x is the
     * destination value and t is the time to destination, finishing with a single
     * time value to zero. For example the array 1, 0, 1000 would give you an
     * instant attack to 1, followed by a 1s decay to zero. The array 1, 500, 1000
     * would give you a ramp up from 0 to 1 over 500ms followed by a 1s decay to
     * zero. An array with an even number of elements is incorrectly formed, but as
     * a rule, the final value in the array will always dictate the decay time to
     * zero. This object will self-destruct once the envelope has completed. The
     * number of channels of this {@link UGen} is dictated by the number of channels
     * of the src {@link UGen}.
     *
    ...
    /**
     * Create an ADSR with the given number of channels. The array of float
     * arguments should consist of a series of pairs [x,t] where x is the
     * destination value and t is the time to destination, finishing with a single
     * time value to zero. For example the array 1, 0, 1000 would give you an
     * instant attack to 1, followed by a 1s decay to zero. The array 1, 500, 1000
     ...
     * zero. An array with an even number of elements is incorrectly formed, but as
     * a rule, the final value in the array will always dictate the decay time to
     * zero. This object will self-destruct once the envelope has completed.
     * 
     * @param context the {@link AudioContext}.
    ...
    public synchronized void addInput(int inputIndex, UGen sourceUGen, int sourceOutputIndex) {
        /*
         * Forward any incoming connections to the gain. This works for addInput(UGen)
         * too because that one forwards to this method.
         */
