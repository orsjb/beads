...
public class OnePoleFilter extends IIRFilter implements DataBeadReceiver {
    ...
    public void calculateBuffer() {
        ...
        if (isFreqStatic) {
            ...
            for (int currsamp = 0; currsamp < bufferSize; currsamp++) {
                a1 = (b0 = (float) Math.sin(two_pi_over_sf * freqUGen.getValue(0, currsamp))) - 1;
                bo[currsamp] = y1 = b0 * bi[currsamp] - a1 * y1;
            }
    ...
    /**
     * Gets a DataBead with the filter frequency (whether float or UGen), stored in
     * the key "frequency".
     * 
    ...
    public IIRFilterAnalysis getFilterResponse(float freq) {
        return calculateFilterResponse(new float[] { b0 }, new float[] { 1, a1 }, freq, context.getSampleRate());
    }
