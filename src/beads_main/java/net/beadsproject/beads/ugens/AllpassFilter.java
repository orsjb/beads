...
public class AllpassFilter extends IIRFilter implements DataBeadReceiver {
        ...
        if (isDelayStatic && isGStatic) {
            ...
            for (int currsample = 0; currsample < bufferSize; currsample++) {
                ...
                bo[currsample] = yn[ind] = xn[ind2]
                        + gUGen.getValue(0, currsample) * (yn[ind2] - (xn[ind] = bi[currsample]));
     ...
     * Sets the delay.
     *
     * @param del The delay in samples. This will remove the delay UGen if there is
     *            one.
     * @return This filter instance.
     */
    ...
    /**
     * Sets a UGen to determine the delay in samples. Delay times are converted to
     * integers. Passing a null value freezes the delay at its current value.
     *
    ...
    /**
     * Gets a DataBead with properties "delay" and "g" set to static float values
     * corresponding to the current filter parameters.
     *
