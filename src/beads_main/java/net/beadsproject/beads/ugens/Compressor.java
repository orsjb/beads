...
public class Compressor extends UGen implements DataBeadReceiver {
    ...
    /**
     * Constructor for a multi-channel compressor with no look-ahead time and other
     * parameters set to their default values.
     * 
     * @param context  The audio context.
    ...
    /**
     * Constructor for a multi-channel compressor with the specified side-chain, no
    ...
    /**
     * Constructor for a multi-channel compressor with the specified look-ahead time
    ...
    /**
     * Constructor for a multi-channel compressor with the specified look-ahead time
     * and side-chain, and other parameters set to their default values.
     * 
     * @param context        The audio context.
     * @param channels       The number of channels.
     * @param lookAheadDelay The look-ahead time in milliseconds.
    ...
    public Compressor(AudioContext context, int channels, float lookAheadDelay, UGen sideChain) {
        ...
        setSideChain(sideChain).setAttack(1).setDecay(.5f).setRatio(2).setThreshold(.5f).setKnee(.5f);
    }
        ...
        if (channels == 1) {
            ...
            for (int i = 0; i < bufferSize; i++) {
                ...
                if (p <= tok) {
                    ...
                    float x1 = (p - tok) * ikp1 + tok;
                    target = (ktrm1 * x1 + tt1mr) * (p - x1) / (x1 * (knee - 1)) + x1;
                }
    ...
    /**
     * Sets the side chain. The power of the side chain (measured by an {@link RMS}
     * object) determines how much the compressor scales the input; by default (and
     * if this method is passed <code>null</code>), the side chain is the same as
     * the input.
     * 
    ...
    public Compressor setSideChain(UGen sideChain) {
        pf = (new BiquadFilter(context, 1, BiquadFilter.BUTTERWORTH_LP)).setFrequency(31);
        if (sideChain == null) {
     ...
     * compression ratio over the threshold. Values less than 1 will yield expansion
    ...
    /**
     * Sets the compressor parameters with a DataBead. Use the following property
     * names: "threshold", "ratio", "attack", "decay", "knee", and "sidechain".
     * 
