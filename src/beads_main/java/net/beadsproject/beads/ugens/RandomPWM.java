 ...
 * <li>{@link #NOISE} - pulses are distributed continuously between -1 and
 * 1.</li>
 * <li>{@link #SAW} - for random-length ramps between -1 and 1.</li>
 * <li>{@link #RAMPED_NOISE} - for random-length ramps between random values
 * between -1 and 1.</li>
 * <li>{@link #NOISE_ENVELOPE} - for random-length ramps between random values
 * between 0 and 1.</li>
 * </ul>
 *
...
public class RandomPWM extends UGen implements DataBeadReceiver {
    ...
    protected void calcVals() {
        float d = (float) Math.pow(Math.random(), lengthExponent) * lengthDiff + minLength;
        count += d;
        pulseLen = count;
        baseVal = targetVal;
    }
    ...
    /**
     * Sets the pulse length exponent. This parameter controls the distribution of
     * pulse lengths: a value of 1 produces a linear distribution; greater than 1
     * skews the distribution toward the minimum length; less than one skews it
    ...
    /**
     * Use the properties "mode", "minLength", "maxLength", and "lengthExponent" to
     * set the corresponding parameters (type Mode for "mode", floats only for the
     * others).
     */
    public DataBeadReceiver sendData(DataBead db) {
        if (db != null) {
            ...
            setParams(mod, db.getFloat("minLength", minLength), db.getFloat("maxLength", maxLength),
                    db.getFloat("lengthExponent", lengthExponent));
        }
        return this;
    }
