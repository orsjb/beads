 ...
 * 
 * <p>
 * In the simple case, it just wraps N one-channel UGens into one N-channel
 * UGen.
 * 
...
public class MultiWrapper extends UGenChain implements DataBeadReceiver {
     ...
     * channel. {@link #buildUGens(int)} should be implemented to construct a UGen
     * for each channel.
     * 
    ...
    /**
     * Constructor for an n-channel wrapper for UGens with a certain number inputs
     * and a certain number outputs on each channel. {@link #buildUGens(int)} should
     * be implemented to construct a UGen for each channel.
     * 
     * @param context        The audio context.
     * @param channels       The number of channels.
    ...
    /**
     * Constructor for a multi-channel wrapper for an array of UGens that represent
     * separate "channels".
     * 
     * @param context        The audio context.
    ...
    /**
     * Constructs and returns a UGen for each channel. If an array of UGens is not
     * provided to MCWrapper at construction, this method should be overridden with
     * code to create a UGen for each channel. To make the channel UGens modifiable,
     * the UGen should implement DataBeadReceiver properly as this is the only way
     * to change the channel UGens' parameters.
     * 
     * @param channelIndex The index of the channel for which the UGen is being
     *                     created.
    ...
    public void addInput(int channelIndex, int channelUGenInput, UGen sourceUGen, int sourceOutput) {
        addInput(channelIndex * insPerChannel + channelUGenInput, sourceUGen, sourceOutput);
    }
    ...
    /**
     * Forwards Beads to channel UGens. If <code>message</code> is a DataBead and
     * the channel UGens are DataBeadReceivers, it calls
