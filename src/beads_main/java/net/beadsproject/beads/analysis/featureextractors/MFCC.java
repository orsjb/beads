...
/**
 * MFCC generates mel-frequency spectral coefficients, an important feature type
 * in music and speech processing. MFCC receives feature data from a
 * {@link MelSpectrum}, and must be set as a listener to a {@link MelSpectrum},
...
public class MFCC extends FeatureExtractor<float[], float[]> {
    ...
    public MFCC(int numCoeffs) {
        ...
        for (int i = 0; i < numCoeffs; i++) {
            if (i < 9)
                featureDescriptions[i] = "mfcc0" + (i + 1);
            else
                featureDescriptions[i] = "mfcc" + (i + 1);
        }
    }
     ...
     * 
     * @see
     * net.beadsproject.beads.analysis.FeatureExtractor#setNumberOfFeatures(int)
     */
