 ...
 * is set as a listener to a {@link PowerSpectrum} object, not directly from an
 * audio stream.
 * 
 *
 */
public class MelSpectrum extends FeatureExtractor<float[], float[]> {
    ...
    // TODO work out a nice filter so that the spectrum is empty when zero sound
    // comes in
     ...
     * @param numCoeffs  the number of filters to use (number of features).
     */
    ...
    /**
     * Builds the filterbank. Only needs to be called if the size of the input array
    ...
    private void setup() {
        ...
        double melMax = lin2mel((hardMax < sampleRate / 2) ? hardMax : sampleRate / 2); // dpwe 2006-12-11 - hard
                                                                                        // maximum
        ...
        for (int i = 0; i < twiceBufferSize; i++) {
            ...
            if (Double.isInfinite(melOfLin[i]))
                System.out.println("infinte meloflin");
        }
        featureDescriptions = new String[numFeatures];
        for (int i = 0; i < numFeatures; i++) {
            if (i < 9)
                featureDescriptions[i] = "mel0" + (i + 1);
            else
                featureDescriptions[i] = "mel" + (i + 1);
        }
    }
     ...
     * 
     * @see
     * com.olliebown.beads.core.PowerSpectrumListener#calculateFeatures(float[])
     */
