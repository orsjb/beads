...
/**
 * RecordToFile records audio into a file. Based on JavaSound, well, Tritonous,
 * which is based on JavaSound.
 * 
 * You must {@link #kill() kill} this object when finished to finalise the
...
public class RecordToFile extends UGen {
     ...
     * 
     */
    public RecordToFile(AudioContext context, int numberOfChannels, File file, AudioFileFormat.Type type)
            throws IOException {
        ...
        audioFormat = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, context.getSampleRate(),
                16 /* number of bits per sample */, getIns(), 2 * getIns(), /* bytes per frame */
                context.getSampleRate(), false /* big-endian? */
        );

        audioOutputStream = AudioSystemShadow.getAudioOutputStream(type, audioFormat, AudioSystem.NOT_SPECIFIED, file);
    }
     ...
     * @param context          The AudioContext
     * @param numberOfChannels The number of channels
     * @param file             The file to output to. Extension should be .wav.
     * @throws IOException if the audio format is not supported on this machine.
     * 
     */
