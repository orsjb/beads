...
/**
 * Read and write audio files using the Javasound library. Only 'one-shot'
 * reading and writing is supported (i.e. no streaming). See
 * {@link net.beadsproject.beads.data.audiofile.AudioFileWriter} and
 * {@link net.beadsproject.beads.data.audiofile.AudioFileReader}.
 * 
 * @author aengus
 */
public class JavaSoundAudioFile implements AudioFileReader, AudioFileWriter {
    ...
    static public SampleAudioFormat convertJavasoundAudioFormatToBeadsAudioFormat(javax.sound.sampled.AudioFormat af) {
        ...
        SampleAudioFormat newaf = new SampleAudioFormat(af.getSampleRate(), af.getSampleSizeInBits(), af.getChannels(),
                signed, af.isBigEndian());
        return newaf;
    }
    ...
    /**
     * See
     * {@link net.beadsproject.beads.data.audiofile.AudioFileWriter#writeAudioFile}
     */
    public void writeAudioFile(float[][] data, String filename, AudioFileType type, SampleAudioFormat saf)
            throws IOException, OperationUnsupportedException {
        ...
        AudioSystem.write(new AudioInputStream(new ByteArrayInputStream(bytes), jsaf, frames), jsType,
                new File(filename));
    }

    /**
     * See
     * {@link net.beadsproject.beads.data.audiofile.AudioFileWriter#getSupportedFileTypesForWriting}
     */
    ...
    /**
     * See
     * {@link net.beadsproject.beads.data.audiofile.AudioFileReader#readAudioFile}
     */
    ...
    /**
     * See
     * {@link net.beadsproject.beads.data.audiofile.AudioFileReader#getSupportedFileTypesForReading}
     */
    ...
    private void prepareForReading() throws IOException {
        ...
        try {
            ...
            throw (new IOException(e.getMessage())); // converts UnsupportedAudioFileException, which is JavaSound
                                                     // specific, to more generic IOException.
        }

        encodedFormat = encodedStream.getFormat();

        int bitDepth = (encodedFormat.getSampleSizeInBits() == -1) ? 16 : encodedFormat.getSampleSizeInBits();

        decodedFormat = new javax.sound.sampled.AudioFormat(javax.sound.sampled.AudioFormat.Encoding.PCM_SIGNED,
                encodedFormat.getSampleRate(), bitDepth, encodedFormat.getChannels(),
        ...
        if (AudioSystem.isConversionSupported(decodedFormat, encodedFormat)) {
            ...
            if (decodedFormat.getFrameSize() != 2 * decodedFormat.getChannels()) {
                close();
                String s = "Tried to load " + (8 * decodedFormat.getFrameSize() / decodedFormat.getChannels())
    ...
    private void readEntireFile() throws IOException {
        ...
        nFrames = sampleBufferSize / (2 * audioFormat.channels);
     ...
     * interleaved format. It is the responsibility of the caller to interpret this
     * data correctly.
     * 
    ...
    /**
     * Get the values required to decode the raw sample frame data.
     * @return a double array of 2 elements for encoding - {floatOffset, floatScale}
     */
    public double[] getEncodingValues() {
        return new double[] {floatOffset, floatScale};
    }
