...
public class Sample {
    ...
    /**
     * Create a sample from a recourceID. This constructor immediately loads the
     * entire audio file into memory.
     *
     * @throws IOException
     */
    public Sample(File file) throws IOException {
        loadWAVAudioFile(file);
    }
     ...
     * @throws IOException
     */
    public Sample(InputStream fileInputStream) throws IOException {
        loadWAVAudioFile(fileInputStream);
    }
     ...
     *
     */
    private void loadWAVAudioFile(File file) throws IOException {
        //we have to deal with a bug in Tritonus: JavaSound doesn't accept 24-bit wav but strangely Tritonus
        //interprets 24-bit wavs as mp3s. So we intercept all wavs and send them to the WavFileReaderWriter.
        //In the first instance we can only use the file suffix as a clue to this, not the header.
        Class<? extends AudioFileReader> theRealAudioFileReaderClass = audioFileReaderClass == null ? defaultAudioFileReaderClass : audioFileReaderClass;
     ...
     *
     */
    private void loadWAVAudioFile(InputStream inputStream) throws IOException {
        //we have to deal with a bug in Tritonus: JavaSound doesn't accept 24-bit wav but strangely Tritonus
        //interprets 24-bit wavs as mp3s. So we intercept all wavs and send them to the WavFileReaderWriter.
        //In the first instance we can only use the file suffix as a clue to this, not the header.
        Class<? extends AudioFileReader> theRealAudioFileReaderClass = audioFileReaderClass == null ? defaultAudioFileReaderClass : audioFileReaderClass;
        ...
        try {
            this.theSampleData = audioFileReader.readAudioFile(inputStream);
        } catch (Exception e) {
            throw new IOException("Problem loading file (class=" + audioFileReader.getClass() + ") " + inputStream + ", " + e.getMessage());
        }
        this.sampleRate = audioFileReader.getSampleAudioFormat().sampleRate;
        this.nChannels = theSampleData.length;
        this.nFrames = theSampleData[0].length;
